...
public class Ray {
	...
	// */
	// public List<Ray> getSempleRaysSquare(Point3D target, double radius,Vector
	// vUp) {
	...
	// * @param radius the half width of the Square
	// * @param vUp the normal vector - orthogonal to the normal of the origin point
	// * @param n the dimnension of matrix of the random rays contracted through the
	// pixel
	// * @return list of n*n rays
	// */
	// public List<Ray> getRaysSquare(Point3D target, double radius,Vector vUp,int
	// n) {
	// Vector vRight = vUp.crossProduct(this.dir);
	// List<Ray> rays = new LinkedList<>();
	...
	// * @return sample of 5 rays
	// */
	// public List<Ray> getSempleRaysCircle(Point3D target, double radius,Vector
	// vUp) {
	// Vector vRight = vUp.crossProduct(this.dir);
	// List<Ray> rays = new LinkedList<>();
	// List<Point3D> points = List.of(
	...
	/**
	 * create a beam of rays that start at the origin point of this ray and goes
	 * through each point on the black board, when locating the black board with
	 * 'center' at it center
	 * 
	 * @param bb     the black board
	 * @param center the point to put at the center of the black board when
	 *               generates the points from it
	 * @param vUp    the up direction of the black board
	 * @param vRight the right direction of the black board
	 * @return a beam of rays that start at the origin point of this ray and goes
	 *         through each point on the black board, when locating the black board
	 *         with 'center' at it center
	 */
	public List<Ray> createBeam(BlackBoard bb, Point3D center, Vector vUp, Vector vRight) {
		List<Ray> beam = new LinkedList<>();
		for (Point3D p : bb.generate3dPoints(vUp, vRight, center))
			beam.add(new Ray(origin, p.subtract(origin)));
		return beam;

		// Another possibility using more modern Java:
		//		return bb.generate3dPoints(vUp, vRight, center).stream() //
		//				.map(p -> new Ray(origin, p.subtract(origin))) //
		//				.collect(Collectors.toList());
	}

	/**
	 * create a beam of rays that start at points on the black board and goes
	 * through the point 'through', the center point of the black point is the
	 ...
	 * @return a beam of rays that start at points on the black board and goes
	 *         through the point 'through'
	 */
	public List<Ray> createBeamThrough(BlackBoard bb, Vector vUp, Vector vRight, Point3D through) {
		List<Ray> beam = new LinkedList<>();
		for (Point3D p : bb.generate3dPoints(vUp, vRight, origin)) {
	...
	// */
	// public List<Ray> getRaysCircle(Point3D target, double radius,Vector vUp,int
	// n) {
	// Vector vRight = vUp.crossProduct(this.dir);
	// List<Ray> rays = new LinkedList<>();
	// for (int i = 0; i < n; i++) {
	...
	// }
	/**
	 * finds from a list of GeoPoints the GeoPoint with the closest point to the
	 * origin of this ray
	 * 
